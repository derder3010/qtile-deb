...
sudo apt install -y xserver-xorg xinit vim xorg 
...
git clone https://github.com/linuxdabbler/st && sudo apt install libxft-dev libxcb1-dev -yy && cd st && make
cd
mkdir -p ~/.local/src
mv st/ ~/.local/src
sudo ln -sf ~/.local/src/st/st /usr/local/bin
